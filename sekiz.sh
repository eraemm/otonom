...
declare -a instance_types=("c7a.2xlarge")

# Yeni bölgeler
declare -a regions=("us-east-1" "us-west-2" "eu-north-1" "eu-west-1" "eu-central-1")
...
check_vcpu_limit() {
    ...
    # Eğer limit tam olarak 8 değilse bölgeyi atla
    if (( vcpu_limit != 8 )); then
        echo "vCPU limiti 8 değil. Bölge $region atlanıyor."
        return 1
    fi
